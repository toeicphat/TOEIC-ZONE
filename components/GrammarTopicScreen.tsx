...
import { getGrammarTopicContent, GrammarTopicContent, getGrammarQuizQuestions, GrammarQuestion, getGrammarQuizLevels } from '../services/grammarLibrary';
...
const GrammarTopicScreen: React.FC<GrammarTopicScreenProps> = ({ topic, onBack }) => {
    ...
    useEffect(() => {
        ...
        setSelectedLevel(null); // Reset level when topic changes
        setQuestions([]);
        setUserAnswers({});
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {quizLevels.map(level => (
                    <SelectionCard 
                        key={level}
                        title={level === 'Exercises' ? 'Start Exercises' : `Level ${level}`}
                        description={level === 'Exercises' ? `Multiple-choice questions for ${topic}` : `Multiple-choice questions for the ${level} target score.`}
    ...
    return (
        <div className="container mx-auto px-4 py-12">
            <div className="max-w-6xl mx-auto">
                ...
                {["Đại từ", "Trạng từ", "Tính từ", "Động từ", "Mệnh đề danh từ"].includes(topic) ? (
